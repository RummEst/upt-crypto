...
async function updateChart(chart, selectedCoins, coinDataCache, timeframe={}) {
  ...
  for (const selectedCoin of selectedCoins) {
    if (!coinDataCache[selectedCoin]) {
      ...
      console.log(selectedCoin + " ->"); console.log(outData);
...
async function getLongShortData(symbol, interval, startTime, endTime) {
  let url = `https://open-api.coinglass.com/public/v2/indicator/long_short_symbol?symbol=${symbol}&interval=${interval}&start_time=${startTime}&end_time=${endTime}`;
  try {
...
function storeSelectedCoins() {
  ...
  localStorage.setItem('selectedCoinsLS', JSON.stringify(selectedCoinsArray));
}
...
async function loadSelectedCoins() {
  ...
  const selectedCoinsJSON = localStorage.getItem('selectedCoinsLS');
  if (selectedCoinsJSON) {
    ...
    console.log("restoreCoinSelection -> " + selectedCoinsArray.join("; "));
